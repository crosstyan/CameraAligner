...
IMAGE_FOLDER = Path("/Users/iamzlt/Documents/code/CameraAligner/input/camera")
...
def main():
    ...
    if calibration is None and len(all_image_points) > 0:
        ...
        parameters = {
            "camera_matrix": mtx.tolist(),  # convert numpy arrays to list
            "distortion_coefficients": dist.tolist(),
            "rotation_vectors": [rv.tolist() for rv in rvecs],
            "translation_vectors": [tv.tolist() for tv in tvecs],
        }
        with open(OUTPUT_FOLDER / "calibration.yaml", "w") as file:
